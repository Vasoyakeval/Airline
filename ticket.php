...
<main>
  <?php if(isset($_SESSION['userId'])) {   
    ...
    if(isset($_POST['cancel_but'])) {
        ...
        $sql = 'SELECT * FROM ticket as t, Passenger_profile as p WHERE t.ticket_id=? and t.passenger_id=p.passenger_id';
        $stmt = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($stmt,$sql)) {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
              $stmt = mysqli_stmt_init($conn);
              $sql_ins = 'INSERT INTO cticket (ticket_id,passenger_id,flight_id,user_id,seat_no,cost,class,fullname) VALUES (?,?,?,?,?,?,?,?)';
              $stmt = mysqli_stmt_init($conn);
              if(!mysqli_stmt_prepare($stmt,$sql_ins)) {
                header('Location: ticket.php?error=sqlerror');
                exit();            
              }
              mysqli_stmt_bind_param($stmt,'iiiiiiss',$row['ticket_id'],$row['passenger_id'],$row['flight_id'],$row['user_id'],$row['seat_no'],$row['cost'],$row['class'],$row['f_name']);
              mysqli_stmt_execute($stmt);
